...
def main ():
    ...
    output_df = pd.DataFrame(columns=['subject', 'ses','acf_x','acf_y','acf_z'])
    ...
    for p in participants:
        ...
        for ses in bidslayout.get_sessions(subject=p):
            ...
            afni_command = ['3dFWHMx', '-input', residuals[0].path]
 
            
            try:
                result = subprocess.run(afni_command, check=True, capture_output=True, text=True)
                ...
                output_df = pd.concat([output_df, pd.DataFrame({'subject': [p], 'ses': [ses], 'acf_x': [values[0]], 'acf_y': [values[1]], 'acf_z': [values[2]]})], ignore_index=True)
